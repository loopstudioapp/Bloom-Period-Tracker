...
struct PeriodDateSelectionPreviousScreen: View {
    ...
    @State private var displayedMonth: Date = Calendar.current.date(byAdding: .month, value: -1, to: Date()) ?? Date()
    ...
    var body: some View {
        VStack(spacing: .zero) {
            ...
            PillButton(title: "Next", style: .primary, isEnabled: hasSelection) {
                viewModel.previousPeriodStartDate = startDate
                viewModel.previousPeriodEndDate = endDate
                coordinator.advance()
            }
            .padding(.horizontal, AppTheme.Spacing.lg)
    ...
    private var headerText: some View {
        Text("Tap on the dates of your previous period for even better predictions.")
